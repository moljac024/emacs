...
;;         Bojan Matic <matic.bojan@gmail.com>
;; Maintainer: Bojan Matic <matic.bojan@gmail.com>
;; Keywords: extensions
;; Created: 2009-05-24
;; URL: http://github.com/moljac024/emacs/tree/master
;; Version: 0.3
;;
    ...
    (if (null completion-function)
        (if (and (not (minibufferp))
          ...
          (if smart-tab-using-hippie-expand
              (progn
                (if (not (hippie-expand nil))
                    (smart-tab-fallback)))
            (progn
              (if (not (dabbrev-expand nil)))
              (smart-tab-fallback))))
...
(defun smart-tab-fallback ()
  "Run the command that was previously set to the TAB key, before
smart tab was activated. This works by temporarily disabling
smart-tab mode and checking for the command associated with the
tab key. However, another popular emacs mode - yasnippet,
operates in the same way. In order to avoid these two modes
infinitely falling back on each other, yasnippet mode is disabled
as well and this is hardcoded. If yasnippet is not installed, no
harm is done by setting a local variable either way."
  (interactive)
  (let* ((smart-tab-mode nil)
         (yas/minor-mode nil)
         (key (this-command-keys-vector))
         (command-this-key (key-binding key))
         (command-tab (key-binding (kbd "TAB"))))
    (when (commandp command-tab)
      (call-interactively command-tab)
      )))
